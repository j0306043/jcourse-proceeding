...
番号は「発表会場--セッション番号--発表順」の形式（例：本稿はA--2--1）
とする。
\item タイトルとして1ページ目の中央上部に上から順に、「発表題目」、
「発表者氏名（研究室名）」を書くこと。
\item ページ番号は「発表番号--予稿ごとのページ番号」の形式（例：
このページはA--2--1--1）とし、各ページの中央下部に付けること。
%  \item 原稿の提出日時を厳守すること。
\end{itemize}

以下は必須ではなく、参考程度である。
\begin{itemize}
\item ページ余白は、上下20mm、左右15mmを目安とする。ページ番号は余白領
域に入れて構わない。
...
本スタイルファイルは、p{\LaTeX}以外の{\LaTeX}（{pdf\LaTeX},
{Xe\LaTeX}, {Lua\LaTeX}等の）ユーザを対象に設計したものであるが、
p{\LaTeX}でも一応動作する（はず）。
